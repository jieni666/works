...
<div class="wrap clean_fix">
    ...
    <div class="main clean_fix">
        <section class="aside_nav clean_fix">
            <div class="box">
                <ul class="na">
                    ...
                    <li id="action"><span></span><a href="fankui.php">问题反馈</a></li>
                </ul>
            </div>
        ...
        <section class="g-right clean_fix">
            <div class="library_Terse" style="height: 450px">
                <div class="title">
                    <p>问题反馈</p>
                </div>
                <div class="main">
            <form style="margin: 12px 0 0 30px;" onclick="return false">
                <h3>欢迎反馈问题，您的意见和建议就是我们的动力!</h3>
                <h5>我们会认真查阅您反馈的每一个问题，在这里您可以提出遇到的问题，也可以发表自己的建议和想法。</h5>
                <li>问题与建议描述:</li>
                <textarea style="width: 85%;height: 170px;" id="nr"></textarea><br>
                <li>您的联系方式：</li>
                <li>
                   ...
                   $time=time();
                    ...
                    /* print_r($res)*/
                    ?>
                    <input style="width: 23.5%" type="text" class="inp"  disabled="disabled" placeholder="<?php echo $res[0]['id']; ?>">
                    <input style="width: 23.5%" type="text"  class="inp" user_name="<?php echo $res[0]['user_name']; ?>" disabled="disabled" placeholder="<?php echo $res[0]['user_name']; ?>">
                <input style="width: 34%" type="number" class="inp" disabled="disabled" placeholder="<?php echo $res[0]['user_tel']; ?>">
                </li>

                <li class="butt">
                <button style="width: 54.5%;height:35px;" class="fankui" userid="<?php echo $res[0]['id']; ?>"
                        user_name="<?php echo $res[0]['user_name']; ?>"
                >提交反馈</button>
                <input style="width: 30%;height:35px;" class="butto" type="reset" value="取消">
                </li>
            </form>
                </div>
            </div>
...
<script>
    $(function(){
        $(document).on('click','.butto',function () {
            if(confirm("是否清空")){
                $("#nr").val('');
            }

        })
        $(document).on('click','.fankui',function () {
            var user_name=$(this).attr("user_name");
            var id = $(this).attr("userid");
           var  nr =$("#nr").val();
              /*  alert(nr);*/
            /*var nr = $(this).attr("userid");*/
            fankui(id,user_name,nr)
        })
        function fankui(id,user_name,nr){
            $.ajax({
                url:'./manage/fankui_data.php',
                type:'POST',
                data:{
                    id:id,
                    user_name:user_name,
